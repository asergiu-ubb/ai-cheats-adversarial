...
def main(args):
    ...
    log_file= log_file.replace("/log.txt", f'{args.experiment}/[init]_{args.initial_perturbation}'
                                 ...
                                 f'_[train]_{os.path.basename(args.train_model).replace(".pt", "")}/log.txt')
    ...
    for m_idx, model_path in enumerate(models_paths):
        ...
        if os.path.basename(model_path) == os.path.basename(args.train_model):
            train_idx = m_idx
    if train_idx == -1:
        if "yolo" in args.train_model:
            model = YOLO(args.train_model, verbose=False).to(DEVICE)
        else:
            model = RTDETR(args.train_model).to(DEVICE)
        all_models.append(model)
        train_idx = len(all_models)-1
    ...
    for test_idx, test_model_path in enumerate(models_paths):
        if args.only_rtdetr:
            if "rtdetr" not in test_model_path.lower():
                continue
        ...
        for batch_idx in range(num_batches):
            ...
            optimizer = torch.optim.Adam([{'params': noise_model_copy.noise}], lr=0.001)
            # Generate adversarial examples using tog_vanishing_with_update
            img_adv, queries, succ, t = tog_vanishing_with_update(img_tensor, train_idx, noise_model_copy, optimizer, n_iter=args.num_iterations, eps=epsilon, lr=args.learning_rate, noise_function = args.perturbation_mode, only_universal=  args.only_universal)
            queries_list.append(queries)
            if args.perturbation_mode == "get_adv_mask":
                preds = noise_model_copy.forward_mask(img_tensor,test_idx)
            elif args.perturbation_mode == "get_adv":
            ...
            logger.info(f"Current Mean mAP (Clean): {np.mean(maps_clean):.4f}")
            logger.info(f"Current Mean mAP (Adv): {np.mean(maps_adv):.4f}")
            logger.info(f"Current Success Rate: {np.mean(success_rates):.4f}")
            logger.info(f"Current Average Queries: {np.mean(queries_list):.2f}")
            
        # Final results
        total_time = time.time() - start_time
        logger.info(f"Experiment completed. FINAL RESULTS FOR MODEL {test_model_path}")
        logger.info(f"Total time: {total_time:.2f} seconds")
        logger.info(f"Final Mean mAP (Clean): {np.mean(maps_clean):.4f}")
        logger.info(f"Final Mean mAP (Adv): {np.mean(maps_adv):.4f}")
        logger.info(f"Final Success Rate: {np.mean(success_rates):.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--perturbation-mode", choices=['get_adv', 'get_adv_sign', "get_adv_mask"], required=True, help="Perturbation mode to use during testing.")
    ...
    parser.add_argument("--only_universal", action='store_true', help="Only test the universal")
